...
?>
          ...
          <td>
            <form action="{{ route('propietarios.destroy', $propietario) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
                <a href="{{route('propietarios.show',$propietario)}}" class="btn btn-primary"><span class="glyphicon glyphicon-eye-open"></span></a>
                <a href="{{route('propietarios.edit',$propietario)}}" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>
